...
function relativeFrom(path7) {
  const normalized = normalizeSeparators(path7);
  if (fs.isRooted(normalized)) {
    throw new Error(`Internal Error: relativeFrom(${path7}): path is not relative`);
  }
  return normalized;
}
...
function isRoot(path7) {
  return fs.isRoot(path7);
}
...
function isMethodCallOf(callExpression, memberName) {
  ...
  if (expression.kind === ts2.SyntaxKind.PropertyAccessExpression) {
    ...
    if (name.kind == ts2.SyntaxKind.Identifier) {
...
function everyNodeChild(node, cb) {
  return !ts2.forEachChild(node, (node2) => !cb(node2));
}
...
function getSourceFileOfNode(node) {
  while (node && node.kind != ts2.SyntaxKind.SourceFile) {
    node = node.parent;
  }
  return node;
}
function sourceInfo(node, sourceFile) {
  if (node) {
    ...
    if (sourceFile) {
      return ts2.getLineAndCharacterOfPosition(sourceFile, node.getStart(sourceFile));
    }
  }
  return {};
}
...
var Evaluator = class {
  ...
  nameOf(node) {
    if (node && node.kind == ts2.SyntaxKind.Identifier) {
      return node.text;
    }
  ...
  isFoldableWorker(node, folding) {
    if (node) {
      switch (node.kind) {
        case ts2.SyntaxKind.ObjectLiteralExpression:
          return everyNodeChild(node, (child) => {
            if (child.kind === ts2.SyntaxKind.PropertyAssignment) {
        ...
        case ts2.SyntaxKind.CallExpression:
          ...
          return false;
        case ts2.SyntaxKind.NoSubstitutionTemplateLiteral:
        ...
        case ts2.SyntaxKind.TemplateMiddle:
        case ts2.SyntaxKind.TemplateTail:
          return true;
        case ts2.SyntaxKind.ParenthesizedExpression:
        ...
        case ts2.SyntaxKind.BinaryExpression:
          ...
          switch (binaryExpression.operatorToken.kind) {
            ...
            case ts2.SyntaxKind.AsteriskToken:
            case ts2.SyntaxKind.SlashToken:
            case ts2.SyntaxKind.PercentToken:
            case ts2.SyntaxKind.AmpersandAmpersandToken:
            case ts2.SyntaxKind.BarBarToken:
        ...
        case ts2.SyntaxKind.ElementAccessExpression:
        ...
        case ts2.SyntaxKind.Identifier:
          ...
          break;
        case ts2.SyntaxKind.TemplateExpression:
    ...
    switch (node.kind) {
      case ts2.SyntaxKind.ObjectLiteralExpression:
        ...
        ts2.forEachChild(node, (child) => {
          switch (child.kind) {
            case ts2.SyntaxKind.ShorthandPropertyAssignment:
            case ts2.SyntaxKind.PropertyAssignment:
              const assignment = child;
              if (assignment.name.kind == ts2.SyntaxKind.StringLiteral) {
      ...
      case ts2.SyntaxKind.CallExpression:
        ...
        if (isCallOf(callExpression, "forwardRef") && arrayOrEmpty(callExpression.arguments).length === 1) {
          ...
          if (firstArgument.kind == ts2.SyntaxKind.ArrowFunction) {
      ...
      case ts2.SyntaxKind.NewExpression:
      ...
      case ts2.SyntaxKind.TypeReference:
        ...
        const getReference = (node2) => {
          if (typeNameNode.kind === ts2.SyntaxKind.QualifiedName) {
      ...
      case ts2.SyntaxKind.UnionType:
        ...
        const references = unionType.types.filter((n) => n.kind !== ts2.SyntaxKind.UndefinedKeyword && !(ts2.isLiteralTypeNode(n) && n.literal.kind === ts2.SyntaxKind.NullKeyword)).map((n) => this.evaluateNode(n));
        let candidate = null;
      ...
      case ts2.SyntaxKind.TypeAssertionExpression:
        const typeAssertion = node;
        return this.evaluateNode(typeAssertion.expression);
      case ts2.SyntaxKind.PrefixUnaryExpression:
        ...
        if (isDefined(left) && isDefined(right)) {
          if (isPrimitive(left) && isPrimitive(right))
            switch (binaryExpression.operatorToken.kind) {
              ...
              case ts2.SyntaxKind.AmpersandAmpersandToken:
                return left && right;
              case ts2.SyntaxKind.AmpersandToken:
                return left & right;
              case ts2.SyntaxKind.BarToken:
                return left | right;
              case ts2.SyntaxKind.CaretToken:
                return left ^ right;
              ...
              case ts2.SyntaxKind.ExclamationEqualsEqualsToken:
                return left !== right;
              case ts2.SyntaxKind.LessThanToken:
                return left < right;
              case ts2.SyntaxKind.GreaterThanToken:
                return left > right;
              case ts2.SyntaxKind.LessThanEqualsToken:
                return left <= right;
              case ts2.SyntaxKind.GreaterThanEqualsToken:
                return left >= right;
              case ts2.SyntaxKind.LessThanLessThanToken:
                return left << right;
              case ts2.SyntaxKind.GreaterThanGreaterThanToken:
                return left >> right;
              case ts2.SyntaxKind.GreaterThanGreaterThanGreaterThanToken:
                return left >>> right;
              case ts2.SyntaxKind.PlusToken:
                return left + right;
              case ts2.SyntaxKind.MinusToken:
      ...
      case ts2.SyntaxKind.ArrowFunction:
        return recordEntry(errorSymbol("Lambda not supported", node), node);
      case ts2.SyntaxKind.TaggedTemplateExpression:
      ...
      case ts2.SyntaxKind.AsExpression:
        const asExpression = node;
        return this.evaluateNode(asExpression.expression);
      case ts2.SyntaxKind.ClassExpression:
...
function isPropertyAssignment(node) {
  return node.kind == ts2.SyntaxKind.PropertyAssignment;
}
var empty = ts2.createNodeArray();
...
var Symbols = class {
  ...
  buildImports() {
    ...
    const visit2 = (node) => {
      switch (node.kind) {
        case ts3.SyntaxKind.ImportEqualsDeclaration:
          const importEqualsDeclaration = node;
          if (importEqualsDeclaration.moduleReference.kind === ts3.SyntaxKind.ExternalModuleReference) {
          ...
          break;
        case ts3.SyntaxKind.ImportDeclaration:
          ...
          if (bindings) {
            switch (bindings.kind) {
              case ts3.SyntaxKind.NamedImports:
                ...
                break;
              case ts3.SyntaxKind.NamespaceImport:
    ...
    if (this.sourceFile) {
      ts3.forEachChild(this.sourceFile, visit2);
    }
  }
};
...
var MetadataCollector = class {
  ...
  getMetadata(sourceFile, strict = false, substituteExpression) {
    ...
    function maybeGetSimpleFunction(functionDeclaration) {
      if (functionDeclaration.name && functionDeclaration.name.kind == ts4.SyntaxKind.Identifier) {
        const nameNode = functionDeclaration.name;
        const functionName = nameNode.text;
        const functionBody = functionDeclaration.body;
        if (functionBody && functionBody.statements.length == 1) {
          const statement = functionBody.statements[0];
          if (statement.kind === ts4.SyntaxKind.ReturnStatement) {
    ...
    function classMetadataOf(classDeclaration) {
      ...
      if (classDeclaration.heritageClauses) {
        classDeclaration.heritageClauses.forEach((hc) => {
          if (hc.token === ts4.SyntaxKind.ExtendsKeyword && hc.types) {
      ...
      for (const member of classDeclaration.members) {
        ...
        switch (member.kind) {
          ...
          case ts4.SyntaxKind.MethodDeclaration:
            isConstructor = member.kind === ts4.SyntaxKind.Constructor;
            const method = member;
            if (isStatic(method)) {
            ...
            break;
          case ts4.SyntaxKind.PropertyDeclaration:
          case ts4.SyntaxKind.GetAccessor:
          case ts4.SyntaxKind.SetAccessor:
    ...
    ts4.forEachChild(sourceFile, (node) => {
      switch (node.kind) {
        case ts4.SyntaxKind.ExportDeclaration:
          const exportDeclaration = node;
          const { moduleSpecifier, exportClause } = exportDeclaration;
          if (!moduleSpecifier && exportClause && ts4.isNamedExports(exportClause)) {
    ...
    const isExport = (node) => sourceFile.isDeclarationFile || ts4.getCombinedModifierFlags(node) & ts4.ModifierFlags.Export;
    ...
    ts4.forEachChild(sourceFile, (node) => {
      switch (node.kind) {
        case ts4.SyntaxKind.ClassDeclaration:
          ...
          break;
        case ts4.SyntaxKind.InterfaceDeclaration:
    ...
    ts4.forEachChild(sourceFile, (node) => {
      switch (node.kind) {
        case ts4.SyntaxKind.ExportDeclaration:
          ...
          const { moduleSpecifier, exportClause } = exportDeclaration;
          if (!moduleSpecifier) {
            if (exportClause && ts4.isNamedExports(exportClause)) {
              exportClause.elements.forEach((spec) => {
          ...
          if (moduleSpecifier && moduleSpecifier.kind == ts4.SyntaxKind.StringLiteral) {
        ...
        case ts4.SyntaxKind.ClassDeclaration:
          ...
          break;
        case ts4.SyntaxKind.TypeAliasDeclaration:
        ...
        case ts4.SyntaxKind.FunctionDeclaration:
          ...
          break;
        case ts4.SyntaxKind.EnumDeclaration:
          ...
          if (isExported3(enumDeclaration)) {
            ...
            for (const member of enumDeclaration.members) {
              ...
              let name = void 0;
              if (member.name.kind == ts4.SyntaxKind.Identifier) {
        ...
        case ts4.SyntaxKind.VariableStatement:
          ...
          for (const variableDeclaration of variableStatement.declarationList.declarations) {
            if (variableDeclaration.name.kind == ts4.SyntaxKind.Identifier) {
              ...
              const report = (nameNode) => {
                switch (nameNode.kind) {
                  case ts4.SyntaxKind.Identifier:
                    ...
                    break;
                  case ts4.SyntaxKind.BindingElement:
                    const bindingElement = nameNode;
                    report(bindingElement.name);
                    break;
                  case ts4.SyntaxKind.ObjectBindingPattern:
                  case ts4.SyntaxKind.ArrayBindingPattern:
...
function namesOf(parameters) {
  ...
  function addNamesOf(name) {
    if (name.kind == ts4.SyntaxKind.Identifier) {
...
var CompilerHostAdapter = class {
  ...
  getMetadataFor(fileName, containingFile) {
    const { resolvedModule } = ts5.resolveModuleName(fileName, containingFile, this.options, this.host);
    let sourceFile;
    if (resolvedModule) {
      ...
      sourceFile = this.host.getSourceFile(resolvedFileName, ts5.ScriptTarget.Latest);
    } else {
      if (!this.host.fileExists(fileName + ".ts"))
        return void 0;
      sourceFile = this.host.getSourceFile(fileName + ".ts", ts5.ScriptTarget.Latest);
    }
    if (!sourceFile) {
      return void 0;
    } else if (this.cache) {
...
function createSyntheticIndexHost(delegate, syntheticIndex) {
  ...
  newHost.getSourceFile = (fileName, languageVersion, onError) => {
    if (normalize2(fileName) == normalSyntheticIndexName) {
      const sf = ts6.createSourceFile(fileName, syntheticIndex.content, languageVersion, true);
...
function createCompilerHost({ options, tsHost = ts8.createCompilerHost(options, true) }) {
...
var TsCompilerAotCompilerTypeCheckHostAdapter = class {
  constructor(rootFiles, options, context, metadataProvider, codeGenerator, librarySummaries = new Map()) {
    ...
    this.moduleResolutionCache = ts8.createModuleResolutionCache(this.context.getCurrentDirectory(), this.context.getCanonicalFileName.bind(this.context));
  ...
  resolveModuleName(moduleName, containingFile) {
    const rm = ts8.resolveModuleName(moduleName, containingFile.replace(/\\/g, "/"), this.options, this, this.moduleResolutionCache).resolvedModule;
  ...
  getOriginalSourceFile(filePath, languageVersion, onError) {
    ...
    if (!languageVersion) {
      languageVersion = this.options.target || ts8.ScriptTarget.Latest;
    }
...
New: ${Array.from(newRefs)}`);
  ...
  addGeneratedFile(genFile, externalReferences) {
    ...
    const sf = ts8.createSourceFile(genFile.genFileUrl, sourceText, this.options.target || ts8.ScriptTarget.Latest);
    if (this.options.module === ts8.ModuleKind.AMD || this.options.module === ts8.ModuleKind.UMD) {
    ...
    if (!sf) {
      ...
      if (summary) {
        if (!summary.sourceFile) {
          summary.sourceFile = ts8.createSourceFile(fileName, summary.text, this.options.target || ts8.ScriptTarget.Latest);
        }
...
function makeDiagnostic(code, node, messageText, relatedInformation) {
  node = ts10.getOriginalNode(node);
  return {
    category: ts10.DiagnosticCategory.Error,
    code: ngErrorCode(code),
    file: ts10.getOriginalNode(node).getSourceFile(),
...
var SemanticDepGraph = class {
  ...
  getSymbolByName(path7, identifier) {
    if (!this.files.has(path7)) {
      return null;
    }
    const file = this.files.get(path7);
...
function normalizeSeparators3(path7) {
  return path7.replace(/\\/g, "/");
}
